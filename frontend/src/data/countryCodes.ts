...
export const DIAL_CODE_OPTIONS = Array.from(new Set(COUNTRY_CODES.map((c) => c.dial))).sort(
  (a, b) => Number(a.slice(1)) - Number(b.slice(1))
)

/** Splits a stored "+91 9876543210" style value into its dial code and local number. */
export function splitMobile(raw: string): { code: string; number: string } {
  const trimmed = raw.trim()
  if (!trimmed.startsWith('+')) return { code: '', number: trimmed }
  const spaceIdx = trimmed.indexOf(' ')
  if (spaceIdx === -1) return { code: '', number: trimmed }
  const codePart = trimmed.slice(0, spaceIdx)
  if (!DIAL_CODE_OPTIONS.includes(codePart)) return { code: '', number: trimmed }
  return { code: codePart, number: trimmed.slice(spaceIdx + 1).trim() }
}
